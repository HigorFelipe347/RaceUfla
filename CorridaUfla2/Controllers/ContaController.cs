...
namespace CorridaUfla2.Controllers
{
    public class ContaController : Controller
    {
        ...
        {
            var corredor = DadosApp.Corredores.FirstOrDefault(u => u.Email == email && u.Senha == senha);
            if (corredor != null)
            {
                SetSession(corredor.Id, corredor.Nome, TipoUsuario.Corredor);
                return RedirectToAction("Index", "Home");
            }
            var org = DadosApp.Organizadores.FirstOrDefault(u => u.Email == email && u.Senha == senha);
            if (org != null)
            {
